...
passport.use(new LocalStrategy(money.authenticate()));
...
router.post('/signup',async function(req, res, next) {
 // money.create(req.body).then(()=>res.redirect('/signin')).catch((err)=>res.send(err));
  try {
    const data=await money.register(
...
router.post("/send-mail", async function (req, res, next) {
  try {
      const user = await money.findOne({ email: req.body.email });
      if (!user)
...
router.post("/forget/:id", async function (req, res, next) {
  try {
      const user = await money.findById(req.params.id);
...
router.get("/update/:id",async function (req, res, next) {
  try {
   const add= await Expense.findById(req.params.id);
    res.render("edit",{ elem: add });
  } catch (error) {
   res.send(error); 
  }
})
